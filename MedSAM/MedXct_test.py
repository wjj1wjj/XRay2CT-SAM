...
def parse_args():
  ...
  parser.add_argument('-mod', type=str, default='sam_lora', help='mod type:sam_lora,sam_adpt,val_ad')
  parser.add_argument('-mid_dim', type=int, default=4 , help='middle dim of adapter or the rank of lora matrix')
  parser.add_argument('--exp', type=str, default='zm_p2', dest='exp',
                     help='exp_name ')
  parser.add_argument('--geometry', type=str, default='2540geometry.xml')
  parser.add_argument('--date', type=str, default='MASA')
  parser.add_argument('--gpu', type=str, default='2', dest='gpuid',
  ...
  parser.add_argument('--experiment_path', type=str, default='/hdd2/wjj/SAMXct/')
  parser.add_argument('--phase',type=str,default='test')
  parser.add_argument('--drr_generation',type=int,default=1)
  parser.add_argument('--arch', type=str, default='SAM_XCT', dest='arch',
...
def test(val_loader, model, criterion,criterionDice, args, tb, mode):
    ...
    with torch.no_grad():
      for i,(input, target,label,image_index) in enumerate(val_loader): #,angle
        ...
        target_var = torch.unsqueeze(target,dim=1)
        ...
        Recon_loss = criterion(output, target_var)
        ...
        gt[i, :, :, :] = target_var.cpu().numpy()
        print('{0}: [{1}/{2}]\t'
              'Recon Loss: {ReconLoss.val:.5f} (average:{ReconLoss.avg:.5f})\t'.format(
               mode, i+1, len(val_loader), 
               ReconLoss = test_recon_loss))

        ###############################################
        to_output = torch.squeeze(output,dim=1)
        generate_CT = to_output.cpu().numpy()
    ...
    save_txt_path = os.path.join(args.experiment_path ,args.exp ,args.date,args.model_name,'metrics.txt')
...
if __name__ == '__main__':
    ...
    #define filefolder
    test_datasetfile = '/hdd2/wjj/experiment/' + 'test.txt' 
    ...
    # define model
    model = SAM_XCT(in_channels=args.num_views, gain=args.init_gain, init_type=args.init_type)
