...
def Structural_Similarity(arr1, arr2, size_average=True, PIXEL_MAX=1.0):
  ...
  ssim_d = []
  for i in range(N):
    ssim = compare_ssim(arr1_d[i], arr2_d[i], data_range=PIXEL_MAX, multichannel=True)
    ssim_d.append(ssim)
  ...
  ssim_h = []
  for i in range(N):
    ssim = compare_ssim(arr1_h[i], arr2_h[i], data_range=PIXEL_MAX, multichannel=True)
    ssim_h.append(ssim)
  ...
  ssim_w = []
  for i in range(N):
    ssim = compare_ssim(arr1[i], arr2[i], data_range=PIXEL_MAX, multichannel=True)
    ssim_w.append(ssim)
...
def getErrorMetrics(im_pred, im_gt, mask=None):
    ...
    # SSIM
    ssim_pred = compare_ssim(im_gt, im_pred,data_range=1.0)
    # MSE
