...
from OurData_augmentation import multi_Data_Augmentation
import xml.etree.ElementTree as ET
import random 
# The CT data used to generate the DRR here is from the Case10Pack_online package
class offline_multiDataSet(Dataset):
  ...
  def __getitem__(self, item):
    ...
    # Generate DRR at any Angle
    rtkct_filepath=os.path.join('/hdd2/wjj/'+self.exp+'/', 'ct_rtk', str(self.dataset_paths[item])+'.mha')
    # rtkct_filepath=os.path.join('/hdd2/wjj/'+self.exp+'/', 'ct_rtk', str(self.dataset_paths[item])+'_rtked'+'.mha')  # case1
    geometry_filepath=os.path.join(self.geometry)
    ...
    while angle1==angle2:
      ...
      angle1=random.randrange(0,360)
      angle2=random.randrange(0,360)
    '''
    first_angle=angle1 if angle1<angle2 else angle2
    arc=angle1 if angle1>angle2 else angle2
    os.environ['CUDA_VISIBLE_DEVICES'] = str(self.id[0])
    os.system(f'/home/zm/XctNet_BM/RTK/rtksimulatedgeometry -f {str(first_angle)} -n 1 --arc {str(arc)} -o {geometry_filepath} && /home/zm/XctNet_BM/RTK/rtkforwardprojections -g {geometry_filepath} -i {rtkct_filepath} -o {drr_filepath} --dimension=128 --fp=CudaRayCast')
    drr_img=sitk.ReadImage(drr_filepath)
    drr_array=sitk.GetArrayFromImage(drr_img)
    drr_array = np.tile(drr_array,(3,1,1))
